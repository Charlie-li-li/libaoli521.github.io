...
import { Button,Modal,Calendar,message,Input,Radio,Spin,TimePicker,DatePicker  } from 'antd';
...
export default class MoreTaskEdit extends React.Component {
    ...
    render() {
        const { editType,modalShow,editVal,tagLoading,tagList,tagIds,taskDateTime } = this.state; 
        ...
        return (
            <div style={{display:'inline'}}>
                ...
                >
                    {editType==='完成时间'?
                    <div style={{ width: '100%',border: '1px solid #d9d9d9', borderRadius: 4 ,display:'flex'}}>
                        <div className="dateBox">
                            {/* <Calendar locale={zhCN} value={moment(editVal, 'YYYY-MM-DD')} fullscreen={false} onSelect={(val)=>{this.valChange('完成时间',val,'')}} /> */}
                            <DatePicker  locale={zhCN}
							format="YYYY-MM-DD"
							placeholder="请选择日期"
							onChange={(date, dateString) => { this.valChange('完成时间',dateString,'') }}/>
                        </div>
                        <div className="timeBox">
                            <TimePicker locale={zhCN}
                                placeholder="请选择时间"
                                disabled={taskDateTime === ''?true:false}
								onChange={(date, dateString) => { this.valChange('完成时间',taskDateTime,dateString) }} 
								format="HH:mm"/>
                        </div>
                    </div>
                    :''} 
