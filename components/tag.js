...
export default class TagComponent extends React.Component {
  ...
  componentWillMount() {
    ...
    if (this.props.showTagTilte) {
      this.setState({ showTagTilte: this.props.showTagTilte });
    }
  }
  componentDidMount() {}
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.showTagTilte === true || nextProps.showTagTilte === false) {
      this.setState({ showTagTilte: nextProps.showTagTilte });
    }
  }
  ...
  render() {
    ...
    return (
      <div className="cpet_tag">
        ...
        <div className="top_title" style={{ justifyContent: "flex-start" }}>
          {showTagTilte ? <span>分类标签</span> : ""}
          {!inputVisible && canEdit && (
            <Popover
              content={this.tagListRender()}
              placement={poverPosition ? poverPosition : "bottom"}
            >
              <Icon
                type="plus-square"
          ...
          {tags.map((tag, index) => {
            ...
            const tagElem = (
              <div
                ...
                className={
                  "ant-tag " +
                  (tag.color ? getTagColorByColorCode("1", tag.color) : "")
                }
              >
