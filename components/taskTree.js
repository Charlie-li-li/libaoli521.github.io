      ...
      >
        ...
        {checkBoxShow ? (
          ...
          >
            <Checkbox
              ref={`box${task.taskId}`}
              checked={
                checkedTaskIds.indexOf(task.taskId) !== -1 ? true : false
              }
              disabled={notCheckIds.indexOf(task.taskId) !== -1 ? true : false}
              // onChange={e => {
              //   this.checkingTask(e.target.checked, task.taskId);
              // }}
            />
          </div>
        ) : (
        ...
        {!taskLiConcise ? (
          <div className="taskList_right">
            ...
            <Icon
              className="attention"
              type={task.attention ? "star" : "star-o"}
              onClick={() => {
                this.attention(task);
              }}
            />
