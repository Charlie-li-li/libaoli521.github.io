...
export default class TaskDetails extends React.Component {
  ...
  handlePreview = file => {
    ...
    // 	previewVisible: true,
    // });
    const files = { fileUrlAbsolute: file.url };
            ...
            {createTask ? (
              <li className="addChildLi">
                <div className="addChild">
                  ...
                  >
                    {newTaskFzr.avatar || newTaskFzr.photo ? (
  ...
  save() {
    const { taskInfo, taskPlanDate, taskPlanTime } = this.state;
    let tag = [];
  ...
  render() {
        ...
        {!isSmall && taskInfo.projectJurisdiction && (
          <Menu.Item>
            <a
              ...
              style={
                !getTeamInfoWithMoney("是否可用") ? { paddingLeft: "30px" } : {}
              }
            >
              删除任务
            </a>
    ...
    return (
      <div className="taskDetails">
        ...
        {addCoopTaskShow ? (
          <TaskAddWithCoop
            task={{ id: taskInfo.id, projectId: taskInfo.project.id }}
        ...
        >
          <div className="objName">
            {taskInfo.bread.length > 0 ? (
              <span className="textMore">{taskInfo.bread[0].taskname}</span>
            ) : (
              ""
            )}
          ...
          >
            ...
            <li>
              ...
              <div
                className="valBox"
                onClick={() => {
                  this.selUser("确认人", jurisdiction);
                }}
              >
                <span className="val textMore" style={{ flex: "0 0 auto" }}>
                  {taskInfo.qrr.name ? taskInfo.qrr.name : "未指派"}
                </span>
                {taskInfo.qrr.name && jurisdiction ? (
                  <Icon
                    type="close-circle"
                    className="close"
                    onClick={e => {
                ...
                >
                  ...
                  {/* {taskPlanTime ==='' && jurisdiction?
										<TimePicker  locale={locale} 
											placeholder="时间"
											disabled={taskPlanDate === ''?true:false}                
											onChange={(date, dateString) => {this.dateTimeChange(taskPlanDate, dateString) }} 
											format="HH:mm"/>
									:''} */}
                  {taskPlanTime === "" && jurisdiction ? <font>时间</font> : ""}
                  <span
                    className="iconBox"
                    onClick={() => {
                      this.clearTime("");
          ...
          >
            ...
            <ul className="disList">
              {taskInfo.talk.map((item, i) => {
                const descriptionx = item.description.split("\n").join("<br>");
                if (item.type === "1") {
                  if (showLog) {
                    if (!item.files) {
                      return (
                  ...
                  const itemPlus = item.description.split("\n").join("<br>");
