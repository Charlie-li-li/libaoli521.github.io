...
export default class TaskList extends React.Component {
  ...
  componentWillMount() {
    ...
    if (
      this.props.taskCheckedShow === true ||
      this.props.taskCheckedShow === false
    ) {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      nextProps.taskCheckedShow === true ||
      nextProps.taskCheckedShow === false
    ) {
        ...
        {taskList.map((item, i) => {
          if (hideTaskIds.indexOf(item.taskinfo.id) == -1) {
            ...
            if (item.taskinfo.state !== "1" && item.taskinfo.state !== "4") {
              ...
              (item.taskinfo.state === "1" || item.taskinfo.state === "4") &&
              hideOkTask
            ) {
              isShow = false;
            } else {
            ...
            if (isShow) {
              return (
                <div
                  ...
                  onClick={() => {
                    if (taskCheckedShow) {
                      this.checking(item.taskinfo.id, i);
                    }
                  }}
                >
                  <div
                  ...
                  {taskCheckedShow ? (
                    <div className="checkBox">
                      <Checkbox
                        ref={`box${i}`}
                        // onChange={(e)=>{this.checking(e,item.taskinfo.id)}}
                        checked={
                          checkTaskIds.indexOf(item.taskinfo.id) !== -1
                            ? true
                            : false
                        }
                        disabled={
                  ...
                  <div
                    ...
                    style={{ left: taskCheckedShow ? "25px" : "" }}
                    onClick={() => {
                      if (!taskCheckedShow) {
                        this.setState({ actTaskId: item.taskinfo.id });
                        this.props.taskClickCallBack(
                          item.taskinfo.id,
                          item.project.id
                        );
                      }
                  ...
                  >
                        ...
                        {item.labels &&
                          item.labels.map(lab => {
                            return (
                              <span
                                key={lab.id}
                                className={getTagColorByColorCode(
                                  "1",
                                  lab.color
                                )}
                              >
                    ...
                    <div className="core_row">
                      ...
                      <span
                        className="textMore"
                        style={{
                          color: stateColorWithTime(
                            item.taskinfo.stateName,
                            item.taskinfo.planEndTime
                          )
                        }}
                      >
                        {item.taskinfo.stateName === "1" ||
                        item.taskinfo.stateName === "8" ||
                        item.taskinfo.stateName === "9"
                          ? item.taskinfo.realityEndTime
                            ? item.taskinfo.realityEndTime.slice(0, 10)
                            : "未设置"
                          : item.taskinfo.planEndTime
                      ...
                      />
                      <span>
                        {item.taskinfo.childSuccess}/{item.taskinfo.childCount}
                      </span>
